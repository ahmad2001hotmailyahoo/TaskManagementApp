class AssignUserTablesController <  ApplicationController
    ...
    def index
        @assignedTasks =  Task.includes(:user, :assign_user_table).where('assign_user_tables.user_id =?', current_user.id).pluck("assign_user_tables.id, tasks.id, tasks.title, tasks.description, tasks.due_date, tasks.status, users.id, users.email, assign_user_tables.user_id")
